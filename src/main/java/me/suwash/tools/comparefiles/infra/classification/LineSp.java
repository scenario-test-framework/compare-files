 ...
 * 改行コード。
 */
public enum LineSp implements Classification {
    /** LF。 */
    LF("LF"),
    /** CR。 */
    CR("CR"),
    ...
    /** 区分値グループMap。 */
    private static final Map<String, LineSp[]> groupValuesMap;
    /** グループ内デフォルト区分値Map。 */
    private static final Map<String, LineSp> groupDefaultMap;
    ...
    static {
        // グループMap
        groupValuesMap = new HashMap<String, LineSp[]>();
        groupValuesMap.put(GROUP_DEFAULT, new LineSp[]{
            LF,
            CR,
            CRLF,
            None
        ...
        // グループ内デフォルト値Map
        groupDefaultMap = new HashMap<String, LineSp>();
        groupDefaultMap.put(GROUP_DEFAULT, LF);
     ...
     * @return デフォルト区分値
     */
    public static LineSp defaultValue() {
     ...
     * @return デフォルトの区分値
     */
    public static LineSp defaultValue(final String group) {
     ...
     * @return 区分値配列
     */
    public static LineSp[] values(final String group) {
     ...
     * @return 区分値
     */
    public static LineSp valueOfByDdId(final String ddId) {
        for (final LineSp curEnum : LineSp.values()) {
     ...
     * @return 区分値
     */
    public static LineSp valueOfByStoreValue(final String storeValue) {
        for (final LineSp curEnum : LineSp.values()) {
    ...
    public static boolean containsName(final String group, final String name) {
        for (final LineSp curEnum : LineSp.values(group)) {
            if (curEnum.name().equals(name)) {
                return true;
            }
        }
     ...
     * @param storeValue 永続化値
     */
    private LineSp(final String storeValue) {
